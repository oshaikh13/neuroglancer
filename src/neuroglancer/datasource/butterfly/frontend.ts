...
export class VolumeChunkSource extends GenericVolumeChunkSource {
  ...
  toString() {
    return `butterfly:volume:${this.hostnames[0]}/${this.key}/${this.channel}/${this.resolution}/${this.encoding}`;
  }
};
...
export class MultiscaleVolumeChunkSource implements GenericMultiscaleVolumeChunkSource {
  ...
  constructor(
    ...
    // TODO: SETUP INFO ENDPOINT FOR BTFLY
  ...
  getSources(chunkManager: ChunkManager) {
    ...
    let imageSize = parseIntVec(vec3.create(), [1024, 1024, 145]);
    let voxelSize = parseFiniteVec(vec3.create(), [6, 6, 6]);
    ...
    let origLowerVoxelBound =
      parseFiniteVec(vec3.create(), [1, 1, 1]); //TODO: Get from server...
    ...
    for (let spec of VolumeChunkSpecification.getDefaults({
      ...
      let cacheKey = stableStringify(
        { 'spec': spec, key: this.key, channel: this.channel, resolution: [1024, 1024, 145] });
      alternatives.push(chunkManager.getChunkSource(
        VolumeChunkSource, cacheKey,
        () => new VolumeChunkSource(
          chunkManager, spec, this.hostnames, this.key, this.channel, "0", encoding)));
    }
...
export function getShardedVolume(hostnames: string[], path: string) {
  // im burning some shit up for sure
  ...
  let key = match;
  let channel = "hi plz dont hurt me";
  let parameters = {};
...
export function getVolume(path: string) {
  ...
  if (match === null) {
    throw new Error(`Invalid butterfly volume path: ${JSON.stringify(path)}`);
  }
